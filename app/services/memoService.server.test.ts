...
import { MemoryMemoService, SQLiteMemoService } from "./memoService.server"
import { Memo } from "~/models/memo"
import { createMemoFromRow } from "~/utils/db.server"

describe('MemoryMemoService', () => {
  let memoService: MemoryMemoService

  beforeEach(async () => {
    memoService = new MemoryMemoService()
    // delete initial memos
    await memoService.archiveMemo("uuid1")
    await memoService.deleteMemo("uuid1")
    await memoService.archiveMemo("uuid2")
    await memoService.deleteMemo("uuid2")
    await memoService.archiveMemo("uuid3")
    await memoService.deleteMemo("uuid3")
  })
  ...
  it('should create a new memo', async () => {
    ...
    const insertedMemos = await memoService.getAllMemos(false)
    expect(insertedMemos[0]).toStrictEqual(newMemo)
  })
